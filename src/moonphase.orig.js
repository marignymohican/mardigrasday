...
//-----Output Functions------------------------------------------------------------
function clearBoth() { document.frm.outFull.value = ""; document.frm.outNew.value = ""; }
function writeNew(  str ) { document.frm.outNew.value  += str; }
function writeFull( str ) { document.frm.outFull.value += str; }

//-----Date Input Functions--------------------------------------------------------
function getExact() { return document.frm.exact.checked; }
function getTZ()    { // handles the radio buttons for time zones
...
function setCurrentYear() { 
	...
	document.frm.current.value =  t.getFullYear();
}
function nextYear() { setYear( getYear() + 1); updateResults(); }
function prevYear() { setYear( getYear() - 1); updateResults(); }
function thisYear() { setCurrentYear();        updateResults(); }
...
function updateResults() {
	...
	clearBoth();
	if (!valiDate()) { return; }
	var yrstr, year = getYear();
	if (year > 0) { yrstr = String( year ) + " A.D. --\n"; }
	else { yrstr = String( Math.abs(year - 1) ) + " B.C. --\n"; }
	writeNew(  "-- New Moons for " + yrstr  );
	writeFull( "-- Full Moons for " + yrstr );
	calcMoonPhase( getYear() );
} // End updateResults
